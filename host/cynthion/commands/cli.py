...
# The prefix which all Cynthion-subcommands start with.
CYNTHION_PREFIX = 'cynthion_'

def looks_like_valid_cynthion_subcommand(directory, executable):
    """
    Returns true iff the given directory/binary pair seem to represent
    a valid Cynthion subcommand.
    ...
    @return True iff the given binary appares to be a valid Cynthion command.
    """

    full_path = os.path.join(directory, executable)

    # Valid Cynthion subcommands start with our prefix.
    if not executable.startswith(CYNTHION_PREFIX):
        return False
    ...
    # Valid Cynthion subcommands are files.
    if not os.path.isfile(full_path):
        return False

    # Valid Cynthion subcommands are executable.
...
def find_all_subcommands():
    """
    Locates all Cynthion subcommands in the user's path -- that is, all
    binaries that start with CYNTHION_PREFIX.
    """
    ...
    # Search each entry in the path for Cynthion subcommands.
    subcommands = {}
    for directory in path_entires:
        ...
        for executable in os.listdir(directory):
            ...
            # If the executable seems to be a Cynthion command, use it.
            if looks_like_valid_cynthion_subcommand(directory, executable):
                ...
                subcommand_name = executable[len(CYNTHION_PREFIX):]
...
def print_usage(argv):
    ...
    # If we don't have argument name information, assume this was called "cynthion"
    name = os.path.basename(argv[0]) if len(argv) else "cynthion"

    print("usage: {} <subcommand>\n".format(name))
    print("Top-level utility for working with Cynthion devices.\n")
    ...
    print("the name cynthion_<subcommand>, and add it to your path. See the (forthcoming)")
    print("documentation for more information.\n")


def main():
    """
    Main routine that delegates all commands to Cynthion subcommands.
    """
    argv = sys.argv[:]
