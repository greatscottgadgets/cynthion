...
While Control transfers are well suited command and status operations they are not well-suited to the exchange of large quantities of data as the per-packet protocol overhead is higher and they can only transfer a packet size of 8 bytes on low speed (1.5Mbps) devices and 64 bytes on full (12Mbps) and high (512Mbps) speed devices.

On the other hand Bulk transfers support a packet size of up to 512 bytes on high speed devices and do not have the protocol overhead of Control transfers.

Let's start by updating our device's descriptors so it can inform the host that it has bulk endpoints available.
...
In the next section we will create a simple streaming memory store module that will allow us to read & write data over our bulk endpoints.


Define Endpoint Functions
-------------------------

We'll implement the gateware to control the endpoints in the next section but we'll first need to come up with a function for our endpoints. This could be any data source and/or sink but for the purposes of this tutorial let's create a (very) simple storage device.

Using the OUT endpoint you can transmit a stream of data from the host to Cynthion to write into a local memory. Then, you can transmit a request from the host to the IN endpoint to retrieve the previously stored data.

This means we'll need some memory we can read and write to, so let's create an `Amaranth Memory component <https://amaranth-lang.org/docs/amaranth/latest/stdlib/memory.html>`__ that lets us use the FPGA's Block RAM for storage.
