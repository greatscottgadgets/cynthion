...
USB is a *host-centric bus*, what this means is that all transfers are initiated by the host irrespective of the direction of the data transfer.
...
Next, we will extend our control endpoints to support two vendor requests: One to set the state of the Cynthion **FPGA LEDs** and another to get the state of the Cynthion **USER BUTTON**.


Extend Default Control Endpoints
--------------------------------

We'll implement our vendor requests by creating a ``VendorRequestHandler`` and adding it to our device's control endpoint:
...
Job done! In the next section we'll add IN and OUT Bulk endpoints to our device.


Exercises
=========

1. Create a vendor request that will disconnect your device from the USB bus.
2. TODO ???
